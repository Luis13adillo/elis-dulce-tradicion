...
**Order status:** **LIVE site with real Stripe payments. Online ordering is RE-ENABLED** (2026-06-16) after the Stripe webhook reliability fix. The `/order` route renders the `Order` wizard again (`src/App.tsx`). The webhook dedup fix is deployed to prod, the production pre-deploy audit was clean, the Stripe LIVE endpoint config was verified, and the full order→payment→webhook flow passed an end-to-end test in Stripe TEST mode on the `elis-dulce-tradicion-staging` project (including the retry/dedup regression). The DB kill switch `business_settings.online_orders_paused` still exists as an emergency server-side pause control (set it `true` to halt new online orders without a redeploy). The local `.env` file contains `pk_test_` keys for dev only — **do not assume those reflect production**.
